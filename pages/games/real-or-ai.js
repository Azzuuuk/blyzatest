...
const GameOverScreenComponent = ({ scores, teams, gameMode, handleNewGame, singlePlayerSubType, highScore, setHighScore, beatTheGame, playSound, sfxRefs }) => {
    const router = useRouter();
    
    useEffect(() => { 
      if (gameMode === 'single' && singlePlayerSubType === 'infinite') {
          const finalScore = scores[teams[0].name] || 0;
    ...
    const { winnerMessage, finalDetails, trophy } = useMemo(() => {
        let maxScore = -1;
        ...
        if (gameMode === 'single') {
            const score = scores[teams[0].name] || 0;
            if (singlePlayerSubType === 'infinite') {
                const isNewHighScore = score > highScore;
                if (beatTheGame) {
                    return { winnerMessage: "You Beat The Game!", finalDetails: `You correctly identified all items! Final Score: ${score}. ${isNewHighScore ? '<strong>New High Score!</strong>' : ''}`, trophy: '👑' };
                }
                return { winnerMessage: "Game Over!", finalDetails: `Your Score: ${score}. ${isNewHighScore ? '<strong>New High Score!</strong>' : `High Score: ${highScore}`}`, trophy: isNewHighScore ? '🏆' : '👍' };
            }
            if (score === MAX_ROUNDS) return { winnerMessage: "Perfect Score!", finalDetails: "You're an expert!", trophy: '🌟' };
            if (score >= Math.ceil(MAX_ROUNDS * 0.6)) return { winnerMessage: "Great Job!", finalDetails: `You got ${score} out of ${MAX_ROUNDS}.`, trophy: '🏆' };
            return { winnerMessage: "Nice Try!", finalDetails: `You got ${score} out of ${MAX_ROUNDS}.`, trophy: '👍' };
        }

        const winners = teams.filter(t => scores[t.name] === maxScore && maxScore > 0).map(t => t.name);
        if (winners.length === 1) return { winnerMessage: `${winners[0]} Wins!`, finalDetails: `With a top score of ${maxScore}!`, trophy: '🏆' };
        if (winners.length > 1) return { winnerMessage: "It's a Tie!", finalDetails: `${winners.join(' & ')} all scored ${maxScore}!`, trophy: '🏆' };
        return { winnerMessage: "Game Complete!", finalDetails: "Better luck next time!", trophy: '🎉' };

    }, [scores, teams, gameMode, singlePlayerSubType, highScore, beatTheGame]);
    ...
    return (
        <div id="game-over-screen" className="screen active">
            ...
            <div id="final-score-display-el" className="score-display">
                {teams.map(team => {
                     const isWinner = winnerMessage.includes(team.name) || (gameMode==='single' && trophy !== '👍' && trophy !== '🎉');
                    return (
                        <div key={team.name} className={`score-card ${isWinner ? 'winner-glow' : ''}`}>
                            <h4>{team.name}</h4>
                            <div className="score-value">{scores[team.name] || 0}</div>
                        </div>
                    );
            ...
            <div className="game-over-buttons-container">
                ...
                <button id="new-game-btn" onClick={() => router.push('/real-or-ai')} className="btn-secondary"><i className="fas fa-home"></i> Main Menu</button>
            </div>
...
export default function RealOrAIPage() {
    ...
    const setupRound = (initialUsedItems = usedItems) => {
        setRoundChoices({});
    ...
    const processAndShowResults = (finalChoices) => {
    ...
    const screenComponents = {
        [GameScreen.START]: <StartScreenComponent {...{ gameMode, setGameMode, playSound, sfxRefs, teams, setTeams, handleStartGame, setSinglePlayerSubType, highScore, showToast }} />,
        [GameScreen.GAME]: <GameScreenComponent {...{ gameMode, singlePlayerSubType, scores, currentRound, curtainsOpen, currentMedia, teams, currentPlayerIndex, handleChoice, buttonsDisabled }} />,
        [GameScreen.RESULTS]: <ResultsScreenComponent {...{ currentMedia, teams, scores, gameMode, singlePlayerSubType, currentRound, setScreen, handleNextRound, roundChoices }} />,
        [GameScreen.GAME_OVER]: <GameOverScreenComponent {...{ scores, teams, gameMode, handleNewGame, singlePlayerSubType, highScore, setHighScore, beatTheGame, playSound, sfxRefs }} />,
    };
    ...
    return (
        <>
            <Head>
                ...
                {/* 
                  IMPORTANT: Link your new CSS file here. 
                  This path assumes your CSS file is at 'public/styles/real-or-ai.css'.
                  If it's in the 'styles' directory at the root, use '/real-or-ai.css'.
                  <link rel="stylesheet" href="/styles/real-or-ai.css" /> 
                */}
            </Head>
            ...
            <div className="game-page-wrapper">
                <div className="hero-background-elements">
                    ...
                    <div className="bg-game-element bg-human-1"><i className="fas fa-male"></i></div>
                    <div className="bg-game-element bg-camera-1"><i className="fas fa-camera-retro"></i></div>
                </div>
