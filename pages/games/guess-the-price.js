...
const GameOverScreenComponent = ({ scores, gameMode, handleNewGame, singlePlayerSubType, highScore, setHighScore }) => {
    ...
    const { winnerMessage, winnerNames } = useMemo(() => {
        ...
        if (maxScore === -1 || winners.length === 0) {
             return { winnerMessage: "Game Over", winnerNames: [] };
        }
        ...
        if (winners.length === 1) {
            ...
            // Create a message for ties, e.g., "It's a Tie between Player 1 & Player 2!"
    ...
    useEffect(() => { 
      ...
      const triggerConfetti = () => {
        if (typeof window === 'undefined' || (winnerNames.length === 0 && gameMode !== 'single')) return; // No confetti if no winner
      ...
      triggerConfetti(); 
    }, [winnerNames]); // Rerun confetti effect if winners change
